...
function Extstudents_navbar({ isOpen, toggleSidebar }) {
  ...
  const isActive = (path) => {
    ...
    if (path === "") return currentPath === "/ext_student";
    return currentPath.endsWith(path) || currentPath === `/ext_student/${path}`;
  };
...
>     
     ...
     />
      ...
      <div className="bg-white px-2">
        ...
        <Link to="Project_detail" className={`${navDiv("Project_detail")} group`}>
          <FileText size={24} className={navIcon("Project_detail")} />
          <p className={navText("Project_detail")}>Project Details</p>
        </Link>

        <Link to="team" className={`${navDiv("team")} group`}>
          <Users size={24} className={navIcon("team")} />
          <p className={navText("team")}>Student Team</p>
        </Link>
...
export default Extstudents_navbar;
