  ...
  async function handleGoogleLogin() {
    try {
      ...
      const res = await axios.post(
        "http://localhost:1234/auth/google-login",
        {
          token: idToken,
        },
        {
      ...
      // Navigate based on role after Google login
      if (role === "student") {
        navigate("/student");
      } else if (role === "teacher") {
        navigate("/teacher");
    ...
    <>
      <div className="login_div">
        <img
          ...
          alt=""
        />
        <p className="Login">Hi, Welcome back</p>
        <br />
        ...
        <div className="lpassdiv" style={{ position: "relative" }}>
          ...
          >
            {showpassword ? (
              <FaEyeSlash style={{ backgroundColor: "white", fontSize: "120%" }} />
            ) : (
              <FaEye style={{ backgroundColor: "white", fontSize: "120%" }} />
            )}
          </span>
          <><p style={{ fontSize: "000" }}>.</p></>
        </div>
