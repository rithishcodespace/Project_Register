              ...
              if (mentoringTeams < 4) {
                ...
                db.query(sql3, [my_id, team_id], (error, result) => {
                  if (error) return next(error);
                    ...
                    let sql4 = "update projects set guide_reg_num = ? where team_id = ?";
                    db.query(sql4,[my_id,team_id],(error,result) => {
                      if(error)return next(error);
...
router.post("/guide/sent_request_to_guide",(req,res,next) => {
    try{
      const {from_team_id,project_id,to_guide_reg_num} = req.body;  // to details
      if (!from_team_id || !project_id || !to_guide_reg_num) {
        return res.status(400).json({ message: "All fields are required" });
      }
      let sql = "insert into guide_requests values(?,?,?)";
      db.query(sql,[from_team_id,project_id,to_guide_reg_num],(error,result) => {
        if(error)return next(error);
