...
router.post("/sub_expert/add_review_details/:request_id/:status/:expert_reg_num",userAuth,(req,res,next) => {
    try{
      const{project_id,project_name,team_lead,review_date,start_time,venue} = req.body;
      const{request_id,status,expert_reg_num} = req.params;
      if(!project_id || !project_name || !team_lead || !review_date || !expert_reg_num || !start_time || !venue || !request_id || !status)
      {
      ...
      db.query(updatequery,[status,request_id],(error,result) => {
        ...
        if(status === 'accept')
        {
          ...
          let sql = "insert into scheduled_reviews(project_id,project_name,team_lead,review_date,start_time,venue,expert_reg_num) values(?,?,?,?,?,?,?)";
          db.query(sql,[project_id,project_name,team_lead,review_date,start_time,venue,expert_reg_num],(error,result) => {
            if(error) return next(error);
